...
const TX_API = "https::/ongtruminternet.gamer.gd/api/lc79";
const MD5_API = "https::/ongtruminternet.gamer.gd/api/lc79md5";
...
async function fetchHistorySafe(apiUrl) {
  const res = await axios.get(apiUrl);
  const raw = res.data;

  const latestRaw = raw[0];
...
app.get("/tx", async (req, res) => {
  try {
    const { history, latestRaw } = await fetchHistorySafe(TX_API);
...
app.get("/md5", async (req, res) => {
  try {
    const { history, latestRaw } = await fetchHistorySafe(MD5_API);
